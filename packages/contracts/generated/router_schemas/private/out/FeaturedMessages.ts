...
export const FeaturedMessages = z
  .object({
    list: z.array(
      z.record(z.string(), z.any()).and(
        z
          .object({
            ...
            creationTimestamp: z.number().int().gte(0).describe("Message creation timestamp"),
            onWall: z.boolean().describe("If the message was on videowall").optional(),
            status: z.enum(["pending", "published", "deleted"]).describe("Message status"),
  ...
  .describe("Event sent to share the list of messages on wall messages");
export type FeaturedMessages = z.infer<typeof FeaturedMessages>;
