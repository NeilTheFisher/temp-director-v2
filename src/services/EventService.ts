...
export class EventService {
  ...
  async getEvents(strMsisdn: string): Promise<any> {
    try {
        const cacheKey =  `{user:${strMsisdn}}:coordinates` // todo
        ...
        const result = events.filter( (event:Event) => {
            if (event.invitationsOnly) {
                const invites = event.invites || []
                console.log("invites:", invites)
                if (undefined == invites.find((i:Invite) => i.recipient == strMsisdn ))
                    return false
            }
        ...
        return { total_events : result.length, events: result}
    } catch (error) {
      console.log(error)
      return null
    }
