		...
		try {
			...
			if (objValidateMsisdn.valid && !objValidateMsisdn.error) {
				...
				if (!user) {
					...
					console.log(
						'RequestManager.odience: user created?:',
						insertSuccess
					)
					const foundUsers = await this.dbManager.fetchData(
						'user',
						'msisdn',
						msisdn,
						'*'
					...
					console.log(
						'RequestManager.odience: just created user: ',
						user
					)
				}
				...
				console.log(
					'RequestManager.odience: ownedGroups: ',
					ownedGroups
				)
				...
				if (ownedGroups.length === 0) {
					...
					console.log(
						'RequestManager.odience: group found: ',
						groups
					)
					if (!groups || groups.length === 0) {
						...
						console.log(
							'RequestManager.odience: Group created?:',
							insertSuccess
						)
						const groupFetched = await this.dbManager.fetchData(
							'group',
							'name',
							`PrivateGroup:[${user?.msisdn}]`,
							'*'
						...
						await Promise.all(rolesToInsert.map(async (role) => {
					...
					const insertGroupUserSuccess =
						await this.dbManager.insertData(
							'group_user',
							['group_id', 'user_id'],
							[String(group?.id), userId]
						)
					console.log(
						`RequestManager.odience: insertGroupUserSuccess?: ${insertGroupUserSuccess}`
					)

					await this.dbManager.updateData('user', 'id', userId, [
				...
				if (
					user &&
                    (!user.otp_created_at ||
                        new Date(user?.otp_created_at).getTime() <
                            new Date().getTime())
				) {
					console.log(
						'RequestManager.odience: otp_created_at was done earlier or is undefined'
					)
					...
					let objResponse = await acsService.updateUser(
						user?.msisdn,
						otp,
						objValidateMsisdn.country_code
					)
					console.log(
						'RequestManager.odience: updateUser result: ',
						objResponse
					)

					if (objResponse.code !== 200) {
						objResponse = await acsService.createUser(
							user?.msisdn,
							otp,
							objValidateMsisdn.country_code
						)
						console.log(
							'RequestManager.odience: createUser result: ',
							objResponse
						)
					}
					...
					return (objResponse.code || 0) !== 200
						? res
							.status(objResponse.code || 500)
							.send(objResponse.message || '')
						: res.send(objResponse.message || '')
				} else {
					return res
						.status(403)
						.send(
							`Provisioning limit reach for MSISDN: ${msisdn} (Retry later)`
						)
				}
			} else {
				return res
					.status(400)
					.send(
						`Odience.index. validateMsisdn: ${msisdn} failed with error: ${objValidateMsisdn.error}`
					)
			}
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		} catch (error: any) {
			console.error(
				`RequestManager.odience: for msisdn: ${
					rawMsisdn || ''
				}, error => ${error.message || ''}`
			)
			return res
	...
	public async getUserInfo(req: Request, res: Response) {
		...
		if (req.headers.authorization) {
			...
			if (!user) {
				...
				return res
					.status(401)
					.json('User not found or not authenticated')
			}
			let usersReported: UserReported[] = []
			const rowsUsersReported: RowDataPacket[] =
			...
			const rowsUsersBlocked: RowDataPacket[] =
				(await this.dbManager.query(
			...
			const rowsUsersBlockedBy: RowDataPacket[] =
				(await this.dbManager.query(
			...
			const promises = userOrganizations.map(async (groupId: number) => {
				...
				const rowAdminRole: RowDataPacket[] =
					(await this.dbManager.query(
						'SELECT * FROM role where name = ?',
						['{"r":"admin","g":' + groupId + '}']
					)) as RowDataPacket[]
				...
				const rowManagerRole: RowDataPacket[] =
					(await this.dbManager.query(
						'SELECT * FROM role where name = ?',
						['{"r":"manager","g":' + groupId + '}']
					)) as RowDataPacket[]
				...
				const rowModeratorRole: RowDataPacket[] =
					(await this.dbManager.query(
						'SELECT * FROM role where name = ?',
