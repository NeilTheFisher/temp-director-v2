...
export class Settings {
	...
	public static PHILLIPS_HUE_ACCESS_TOKEN_EXPIRATION_DATE = 'phillips_hue_access_token_expiration_date'
	public static PHILLIPS_HUE_REFRESH_TOKEN_EXPIRATION_DATE= 'phillips_hue_refresh_token_expiration_date'
	...
	public static TRIGGER_ACTIONS = [
		{ on: 'Always On' },
		{ moderator: 'Moderator Triggered' }
	]
	...
	private loadedSettings: Map<string, string>

	constructor(dbManager: DbManager) {
		this.dbManager = dbManager

		this.loadedSettings = new Map<string, string>()
	}

	public async getSystemSetting(settingName: string): Promise<unknown> {
		if (this.loadedSettings.has(settingName)) {
			console.log(`Returning saved value: ${settingName}:${this.loadedSettings.get(settingName)}`)
			return this.loadedSettings.get(settingName)
		}
		console.log(`fetching value for ${settingName} from DB`)
		const value = await this.dbManager.fetchData('setting', 'key', settingName, 'value')
		if (value) {
			this.loadedSettings.set(settingName, value[0].value)
